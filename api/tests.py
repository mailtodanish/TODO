...
class ReadActivityTest(APITestCase):
    ...
    def setUp(self):
        self.superuser = User.objects.create_superuser('john', 'john@snow.com', 'johnpassword')
        self.client.login(username='john', password='johnpassword')
        self.Activity = Activity.objects.create(Title="Activity_Title",Description="Description")
    ...
    def test_can_read_activity_detail(self):
        response = self.client.get(reverse('api-activity-detail', args=[self.Activity.id]))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class UpdateActivityTest(APITestCase):
    ...
    def test_can_update_user(self):
        response = self.client.put(reverse('api-activity-detail', args=[self.Activity.id]), self.data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class DeleteActivityTest(APITestCase):
    ...
    def test_can_delete_user(self):
        response = self.client.delete(reverse('api-activity-detail', args=[self.Activity.id]))
...
class daily_random_revision_item_Test(APITestCase):
    ...
    def test_daily_random_revision_item(self):
        api_response = self.client.get(reverse('daily_random_revision_item'),format='json')
