...
from .models import ApplictaionData, ProjectTask, TaskComment, FavLink
...
class LovCreateForm(forms.ModelForm):
    ...
    '''
    Name = forms.CharField(label='Name', required=False, label_suffix="",
                           widget=forms.TextInput(
                               attrs={'class': 'form-control'}))
    Value = forms.CharField(label='Value', required=False, label_suffix="",
                            widget=forms.TextInput(
                                attrs={'class': 'form-control'}))
    Description = forms.CharField(label='Description', required=False,
                                  label_suffix="", widget=forms.TextInput(
                                      attrs={'class': 'form-control'}))
    IsCategory = forms.BooleanField(label='Is Category', label_suffix="",
                                    widget=forms.CheckboxInput(),
                                    required=False)
    Type = forms.ModelChoiceField(label='Type', required=False,
    ...
    def clean_IsCategory(self):
        ...
        if cleaned_data.get("IsCategory"):
            if cleaned_data.get("Value") != '':
                raise forms.ValidationError("Value should be blank if \
                     isCategory is True.")
            elif cleaned_data.get("Type") is not None:
                raise forms.ValidationError("Type should be blank if \
            isCategory is True.")
...
class ProjectTaskCreateForm(forms.ModelForm):
    ...
    '''
    Task_Title = forms.CharField(label='Title', required=False,
                                 label_suffix="",
                                 widget=forms.TextInput(
                                     attrs={'class': 'form-control'}))
...
class ProjectTaskCommentCreateForm(forms.ModelForm):
    ...
    content = forms.CharField(widget=TinyMCE(attrs={'cols': 100,
                              'rows': 10, 'class': 'form-control'}))
    task = forms.ModelChoiceField(queryset=ProjectTask.objects.all(),
...
class LinkCreateForm(forms.ModelForm):
    ...
    link = forms.URLField(label='link', required=True,
                          ...
                          widget=forms.TextInput(
                              attrs={'class': 'form-control'}))
    name = forms.CharField(label='name', required=True, label_suffix="",
                           widget=forms.TextInput(
                               attrs={'class': 'form-control'}))
